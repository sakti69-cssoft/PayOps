...
`POST /api/merchants/:merchant/incidents/:id/investigation` accepts `{}`. Any authenticated member, including a reader, may investigate an incident they own. Returns `mode`, `summary`, cited `facts`, explicitly unconfirmed `hypotheses`, `missingInformation`, and `operatorChecks`. Each cited item includes `id` and `evidenceId`. Limits: ten requests per user per rolling hour, two in flight per API process, ten snapshots, seven seconds per provider call. Requests and outcomes are audited. 404 conceals unauthorized incidents; 429 indicates a budget/concurrency limit; 503 leaves the ordinary incident view available. See [investigation](investigation.md).
